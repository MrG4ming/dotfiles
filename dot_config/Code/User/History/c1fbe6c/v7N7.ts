...
import { ObjectId, type ModifyResult, DBRef, FindCursor, type DeleteResult } from 'mongodb';
...
export async function deleteJob(jobId: ObjectId): Promise<DeleteResult> {
	const data = await jobs.findOne({ _id: jobId });

	if (data) return jobs.deleteOne({ _id: jobId });
}
