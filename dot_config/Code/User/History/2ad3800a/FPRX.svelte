<script>
	import CheckboxInput from '$lib/InputFields/CheckboxInput.svelte';
...
<form method="POST" action="?/login" class="flex justify-center items-center h-full w-full">
	<div class="card shadow-lg dark:shadow-cyan-500 p-4 w-96">
		...
		<div class="p-1">
			<CheckboxInput label="Stay logged in" name="stayLoggedIn" />
		</div>
		...
		<div
			class="pt-5 text-blue-700/60 hover:text-blue-700 dark:text-blue-400/60 dark:hover:text-blue-400 flex justify-center"
		>
			<a href="/register" data-sveltekit-preload-data>I don't have an account?</a>
		</div>
	</div>
</form>
